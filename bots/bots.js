(function() {
    ...
    async function loadServices() {
        try {
            ...
            if (links.length === 0) {
                grid.innerHTML = '<div class="no-results">Нет ботов</div>';
                return;
            }
            ...
            document.querySelectorAll('.service-card').forEach(card => {
                card.addEventListener('click', (e) => {
                    ...
                    else if (url === '#') showMessage(`🔹 ${card.querySelector('.card-title').textContent} — в разработке`);
                });
            });
            ...
            document.querySelectorAll('.card-link').forEach(link => {
                link.addEventListener('click', (e) => {
                    ...
                    else if (url === '#') showMessage(`🔹 ${link.closest('.service-card').querySelector('.card-title').textContent} — в разработке`);
                });
            });
        } catch (err) {
